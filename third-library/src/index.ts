...
interface RentManager {
    getBooks(): Book[]; // 도서관의 현재 도서목록을 확인하는 함수
    addBook(user: User, book: Book): void; // 사서가 도서관에 새로운 도서를 입고할때 호출하는 함수
    removeBook(user: User, book: Book): void; // 사서가 도서관에서 도서를 폐기할 떄 호출하는 함수
...
class Library implements RentManager {
    ...
    getBooks(): Book[] {
        ...
        //   return this.books; // 이게더 편할 수 있지만, 호출받은 객체가 마음대로 변조가 가능하기 때문에 방지하고자 깊은 복사를 한다.
    }
